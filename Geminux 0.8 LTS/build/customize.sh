#!/usr/bin/env bash
# ==============================================================================
# Geminux OS 0.8 LTS (Ubuntu 24.04 LTS Noble) - Live Customization Hook
# Executado dentro do chroot para aplicar a identidade do Geminux 0.8 LTS
# ==============================================================================

set -e
export DEBIAN_FRONTEND=noninteractive

echo "==> [Geminux 0.8 LTS Hook] Configurando Sistema..."
...
if [ -f /tmp/geminux-build/config/gsettings/99_geminux_0.8.gschema.override ]; then
    mkdir -p /usr/share/glib-2.0/schemas
    cp /tmp/geminux-build/config/gsettings/99_geminux_0.8.gschema.override /usr/share/glib-2.0/schemas/
    glib-compile-schemas /usr/share/glib-2.0/schemas/ || true
    echo "    ✓ GSettings override compilado (Prius Terminal na barra de tarefas)"
fi

# 5. Configuração de Repositórios do Ubuntu 24.04 LTS (Noble Numbat)
...
# 6. Instalação e Montagem dos Aplicativos Nativos do Geminux 0.8 LTS
echo "==> Instalando e configurando os aplicativos nativos do Geminux 0.8 LTS..."
...
Archive: noble
Origin: Geminux
Label: Geminux OS 0.8 LTS
Suite: noble
Codename: noble
Architectures: all amd64
Components: main
Description: Geminux OS 0.8 LTS Official Local Repository
EOF_REL
)
...
if [ -f /tmp/geminux-build/apps/geminux-vm/geminux-virtual-machine_1.0.0_all.deb ]; then
    echo "    -> Instalando Geminux VM (.deb)..."
    ...
    if [ -f /tmp/geminux-build/apps/geminux-vm/geminux-vm.metainfo.xml ]; then
        install -m 644 /tmp/geminux-build/apps/geminux-vm/geminux-vm.metainfo.xml /usr/share/metainfo/geminux-vm.metainfo.xml
    fi
fi
...
echo "==> Configurando Fastfetch / Neofetch com logotipo oficial do Geminux OS..."
...
# Geminux OS Official Fetch Configuration
...
if [ -d /usr/share/plymouth/themes/ubuntu-text ]; then
    sed -i 's/title=Ubuntu.*/title=Geminux OS 0.8 LTS/g' /usr/share/plymouth/themes/ubuntu-text/ubuntu-text.plymouth 2>/dev/null || true
fi
...
echo "==> Atualizando initramfs com os temas oficiais do Geminux 0.8 LTS..."
if [ -x "$(command -v update-initramfs)" ]; then
    update-initramfs -u -k all || true
fi

echo "==> [Geminux 0.8 LTS Hook] Finalizado com sucesso!"
